...
def one(old, new, label):
    ...
    c = s.count(old)
    if c != 1:
        raise SystemExit(f"patch {label}: expected 1 match, got {c}")
...
{
    if (!len || !flip_rpi_ensure_uart(app))
    {
        return false;
    }
    char out[BLE_COMMAND_MAX + 2];
    if (len > BLE_COMMAND_MAX)
    {
        len = BLE_COMMAND_MAX;
    }
    memcpy(out, data, len);
...
static void flip_rpi_send_special(FlipRPIApp *app, uint32_t key)
{
    ...
    static const uint8_t ctrl_x[] = {0x18};
    static const uint8_t ctrl_z[] = {0x1A};
    static const uint8_t ctrl_d[] = {0x04};
    static const uint8_t ctrl_l[] = {0x0C};
    static const uint8_t ctrl_u[] = {0x15};
    static const uint8_t ctrl_w[] = {0x17};
    static const uint8_t ctrl_r[] = {0x12};
    static const uint8_t tab[] = {0x09};
    static const uint8_t esc[] = {0x1B};
    static const uint8_t enter[] = {0x0A};
    static const uint8_t backspace[] = {0x7F};
    static const uint8_t up[] = {0x1B, '[', 'A'};
    static const uint8_t down[] = {0x1B, '[', 'B'};
    ...
    switch (key)
    {
    case 0: d = ctrl_c; n = 1; break;
    case 1: d = ctrl_x; n = 1; break;
    case 2: d = ctrl_z; n = 1; break;
    case 3: d = ctrl_d; n = 1; break;
    case 4: d = ctrl_l; n = 1; break;
    case 5: d = ctrl_u; n = 1; break;
    case 6: d = ctrl_w; n = 1; break;
    case 7: d = ctrl_r; n = 1; break;
    case 8: d = tab; n = 1; break;
    case 9: d = esc; n = 1; break;
    case 10: d = enter; n = 1; break;
    case 11: d = backspace; n = 1; break;
    case 12: d = up; n = 3; break;
    case 13: d = down; n = 3; break;
    case 14: d = left; n = 3; break;
    case 15: d = right; n = 3; break;
    default: break;
    }

    if (d)
    {
        flip_rpi_send_raw(app, d, n);
    }
}

static int flip_rpi_alias(const char *x)
{
    if (!strcmp(x, "!CTRL-C") || !strcmp(x, "@key ctrl-c")) return 0;
    if (!strcmp(x, "!CTRL-X") || !strcmp(x, "@key ctrl-x")) return 1;
    if (!strcmp(x, "!CTRL-Z") || !strcmp(x, "@key ctrl-z")) return 2;
    if (!strcmp(x, "!CTRL-D") || !strcmp(x, "@key ctrl-d")) return 3;
    if (!strcmp(x, "!CTRL-L") || !strcmp(x, "@key ctrl-l")) return 4;
    if (!strcmp(x, "!CTRL-U") || !strcmp(x, "@key ctrl-u")) return 5;
    if (!strcmp(x, "!CTRL-W") || !strcmp(x, "@key ctrl-w")) return 6;
    if (!strcmp(x, "!CTRL-R") || !strcmp(x, "@key ctrl-r")) return 7;
    if (!strcmp(x, "!TAB") || !strcmp(x, "@key tab")) return 8;
    if (!strcmp(x, "!ESC") || !strcmp(x, "@key esc")) return 9;
    if (!strcmp(x, "!ENTER") || !strcmp(x, "@key enter")) return 10;
    if (!strcmp(x, "!BACKSPACE") || !strcmp(x, "@key backspace")) return 11;
    if (!strcmp(x, "!UP") || !strcmp(x, "@key up")) return 12;
    if (!strcmp(x, "!DOWN") || !strcmp(x, "@key down")) return 13;
    if (!strcmp(x, "!LEFT") || !strcmp(x, "@key left")) return 14;
    if (!strcmp(x, "!RIGHT") || !strcmp(x, "@key right")) return 15;
    return -1;
}

static void flip_rpi_send_buffered_command(FlipRPIApp *app)
{
    if (!app->ble_command_len)
    {
        return;
    }
    ...
    if (alias >= 0)
    {
        ...
        flip_rpi_send_special(app, (uint32_t)alias);
        return;
    }
    ...
    {
        flipper_http_send_data(app->fhttp, command);
    }
}

static void flip_rpi_ble_flush_timer_callback(void *context)
{
    FlipRPIApp *a = context;
    if (a && a->view_dispatcher)
    {
        view_dispatcher_send_custom_event(a->view_dispatcher, FlipRPICustomEventBLEFlush);
    }
}
...
static uint16_t flip_rpi_ble_serial_callback(SerialServiceEvent e, void *context)
{
    ...
    if (e.event == SerialServiceEventTypeDataReceived)
    {
        FlipRPIBlePacket p = {0};
        p.size = e.data.size > BLE_RX_PACKET_MAX ? BLE_RX_PACKET_MAX : e.data.size;
        memcpy(p.data, e.data.buffer, p.size);
        if (furi_message_queue_put(a->ble_rx_queue, &p, 0) == FuriStatusOk)
        {
            view_dispatcher_send_custom_event(a->view_dispatcher, FlipRPICustomEventBLE);
        }
    }
...
static bool flip_rpi_ble_start(FlipRPIApp *app)
{
    ...
    ble_profile_serial_set_event_callback(app->ble_serial_profile, BLE_SERIAL_BUFFER_SIZE, flip_rpi_ble_serial_callback, app);
...
static void update_special_submenu(FlipRPIApp *app)
{
    ...
    {
        submenu_add_item(app->submenu_special, names[i], SPECIAL_KEY_START + i, callback_submenu_choices, app);
    }
}

static bool alloc_special_submenu(FlipRPIApp *app)
{
    if (app->submenu_special)
    {
        return true;
    }
    if (!easy_flipper_set_submenu(&app->submenu_special, FlipRPIViewSpecialKeys, "Terminal Keys", callback_to_submenu, &app->view_dispatcher))
    {
        return false;
    }
    submenu_add_item(app->submenu_special, "View Output", FlipRPISubmenuIndexRun, callback_submenu_choices, app);
    submenu_add_item(app->submenu_special, "Send Command", FlipRPISubmenuIndexSend, callback_submenu_choices, app);
    update_special_submenu(app);
    return true;
}

static void free_special_submenu(FlipRPIApp *app)
{
    if (app->submenu_special)
    {
        view_dispatcher_remove_view(app->view_dispatcher, FlipRPIViewSpecialKeys);
        submenu_free(app->submenu_special);
        app->submenu_special = NULL;
    }
}
...
fs = fam.read_text().replace("stack_size=4 * 1024", "stack_size=8 * 1024").replace('fap_version="1.0"', 'fap_version="2.0"')
fam.write_text(fs)
